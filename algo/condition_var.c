...
pthread_cond_t doneFirst = PTHREAD_COND_INITIALIZER, 
               doneSecond = PTHREAD_COND_INITIALIZER, 
               doneThird = PTHREAD_COND_INITIALIZER;
...
void *first (void *args)
{
  while(1)
  {
    ...
    while(turn != First)
      pthread_cond_wait(&doneThird, &lock);
    printf("first\n");
    turn = Second;
    pthread_cond_signal(&doneFirst);
...
void *second (void *args)
{
  while(1)
  {
    pthread_mutex_lock(&lock);
    while(turn != Second)
      pthread_cond_wait(&doneFirst, &lock);
    printf("second\n");
    turn = Third;
    pthread_cond_signal(&doneSecond);
    pthread_mutex_unlock(&lock);
  }
}
void *third (void *args)
{
  while(1)
  {
    pthread_mutex_lock(&lock);
    while(turn != Third)
      pthread_cond_wait(&doneSecond, &lock);
    printf("third\n");
    turn = First;
    pthread_cond_signal(&doneThird);
    pthread_mutex_unlock(&lock);
  }
}
